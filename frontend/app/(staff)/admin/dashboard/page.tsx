...
export default function AdminDashboard() {
  const { data: beds = [] } = useQuery({ queryKey: ['dash-beds'], queryFn: async () => (await (await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v1/beds`, { credentials: 'include' })).json()).data || [], refetchInterval: 30000 });
  const { data: blood = [] } = useQuery({ queryKey: ['dash-blood'], queryFn: async () => (await (await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v1/blood`, { credentials: 'include' })).json()).data || [], refetchInterval: 30000 });
  const { data: medicines = [] } = useQuery({ queryKey: ['dash-medicines'], queryFn: async () => (await (await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v1/medicines`, { credentials: 'include' })).json()).data || [], refetchInterval: 30000 });
  const { data: queues = [] } = useQuery({ queryKey: ['dash-queues'], queryFn: async () => (await (await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v1/queues`, { credentials: 'include' })).json()).data || [], refetchInterval: 5000 });
  const { data: admissions = [] } = useQuery({ queryKey: ['dash-admissions'], queryFn: async () => (await (await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v1/admissions?status=Admitted`, { credentials: 'include' })).json()).data || [], refetchInterval: 30000 });
  const { data: lab = [] } = useQuery({ queryKey: ['dash-lab'], queryFn: async () => (await (await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v1/lab?status=Pending`, { credentials: 'include' })).json()).data || [], refetchInterval: 30000 });
